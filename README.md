...
SRT / KTX 매진 열차를 주기적으로 감시해서 자동으로 좌석을 잡아주는 봇입니다.

**동작 흐름:**
1. **좌석 있음** → 즉시 예약 (reserve)
2. **매진 + 예약대기 가능** → 자동으로 예약대기 신청 (SRT: `reserve_standby` / KTX: `try_waiting=True`)
3. **매진 + 대기 불가** → 다음 사이클 대기

각 단계별로 텔레그램에 알림이 옵니다.

> ⚠️ 봇이 잡은 예약은 **일정 시간 내 결제하지 않으면 자동 취소**됩니다. 알림을 받은 즉시 앱에서 결제하세요.
> 예약대기는 자리가 나면 SMS로 알려옵니다.
